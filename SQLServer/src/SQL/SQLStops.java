    ...
    */
    public ArrayList<Stop> getDataByDateAndHours(String dbo,String dateStart, String dateEnd,String hourStart,String hourEnd)
    {
        ...
        try{
           ...
           else rs = selector.executeQuery("SELECT * FROM " + dbo + " WHERE " + "CAST (StartIdle as date )" + " >='" + dateStart + "'  AND " +
                    "(" + "CAST (StartIdle as date )" + " <= '" + dateEnd + "')" + " AND " +
                    (("CAST (StartIdle as time)" + " NOT BETWEEN '" + hourEnd + "'  AND '" + hourStart+ "' ORDER BY " + "StartIdle")));
            addToArrayList(rs,dataByDate);
        }catch(Exception grabDataException){
            System.out.println("SQLStop.java - getDataByDateAndHours() Exception - " + grabDataException.getMessage());
        }
        return dataByDate;
    }
    //Returns data from dbo by machineId
    public ArrayList<Stop> getDataByMachineId(String dbo,String MachineID)
    {
        ...
        try{
            ...
            System.out.println("SQLStop.java - getDataByMachineId() Exception - " + grabDataException.getMessage());
        }
    ...
    public ArrayList<Stop> getDataByDateAndHoursMachineID(String dbo,String dateStart, String dateEnd,String hourStart,String hourEnd,String MachineID)
    {
        ...
        try{
            ...
            rs = selector.executeQuery("SELECT * FROM "+dbo+" WHERE "+"CAST (StartIdle as date )"+" >='"+dateStart+"'  AND " +
                    ...
                    "(" + "CAST (StartIdle as date )" + " <= '" + dateEnd + "')" + " AND " +
                    (("CAST (StartIdle as time)" + " NOT BETWEEN '" + hourEnd + "'  AND '" + hourStart+ "' AND (MachineId = "+MachineID+")"+" ORDER BY " + "StartIdle")));
            addToArrayList(rs,dataByDateAndMachineID);
        }catch(Exception grabDataException){
            System.out.println("SQLStop.java - getDataByDateAndHoursMachineID() Exception - " + grabDataException.getMessage());
        }
        return dataByDateAndMachineID;
    }
    public ArrayList<Stop> getDataByDateMachineID(String dbo,String dateStart, String dateEnd,String MachineID)
    {
        ArrayList<Stop> dataByDateAndMachineID = new ArrayList<>();
        try{
            if(!connectedToDb) return null;
            selector = (Statement) connector.createStatement();
            rs = selector.executeQuery("SELECT * FROM "+dbo+" WHERE "+"CAST (StartIdle as date )"+" >='"+dateStart+"'  AND " +
                    "("+"CAST (StartIdle as date )"+" <= '"+dateEnd+"')"+" AND (MachineId = "+MachineID+")"+" ORDER BY "+"StartIdle");
            addToArrayList(rs,dataByDateAndMachineID);
        }catch(Exception grabDataException){
            System.out.println("SQLStop.java - getDataByDateMachineID() Exception - " + grabDataException.getMessage());
        }
        return dataByDateAndMachineID;
    } 
