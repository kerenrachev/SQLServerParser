...
public class SQLEmployee extends SQLConnection {
    ...
    public boolean changeName(String dbo, int id, String newName) {
    	try{
            if(!connectedToDb) return false;
            selector = (Statement) connector.createStatement();
            selector.executeUpdate("UPDATE "+ dbo+ " SET nameOfEmployee='"+newName+"' WHERE id=" +id );
    ...
    public boolean changeNumOfEmployee(String dbo, int id,int newEmployeeNum) {
    	try{
            if(!connectedToDb) return false;
            selector = (Statement) connector.createStatement();
            selector.executeUpdate("UPDATE "+ dbo+ " SET numOfEmployee='"+newEmployeeNum+"' WHERE id=" +id);
       }catch(SQLException addEmployeException)
       {
