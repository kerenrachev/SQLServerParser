    ...
    public void analyseData( boolean isMorning) throws ParseException {
        ...
        if (divideByShifts) {
            if (cyclesTableInfo.size() > 0 && !morning)
                cycleFirstDayDivided = checkHour(cyclesTableInfo.get(0).getHour(), morningEnd);
            if (stopTableInfo.size() > 0 && !morning)
                stopFirstDayDivided = checkHour(stopTableInfo.get(0).getHour(), morningEnd);
        }
                ...
                if (!cycleFirstDayDivided) {//check if first day in evening shift
                    ...
                    if (i + 1 == cyclesTableInfo.size()) {//if last day
                        if (currentDate.equals(curr.getDate())||(currentDate.equals(prevDate)&&nightH)) {
                            test=test+curr.getTest()*curr.getTWork();
                            ...
                            test=curr.getTest()*curr.getTWork();
        ...
        for (int i = 0; i < stopTableInfo.size(); i++) {
            ...
            if (!currentDate.equals(current.getDate()) && (morning || check||!morning&&!currentDate.equals(getPrevDay(current.getDate()))) || (stopFirstDayDivided && check) || i + 1 == stopTableInfo.size()) {
